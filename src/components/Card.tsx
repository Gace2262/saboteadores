  ...
  return (
    ...
    >
      <RarityFrame rarity={getRarity(card)} selected={selected} disabled={disabled}>
        <div className={`relative flex h-full flex-col p-3 ${compact ? "min-h-[300px]" : "min-h-[340px] sm:min-h-[420px]"}`}>
          ...
          <div className="flex min-h-14 items-start justify-between gap-3 rounded-t-xl border border-white/10 bg-black/45 p-3 pl-14 sm:min-h-16">
            <div className="min-w-0">
              <p className="truncate text-[10px] font-black uppercase tracking-[0.22em]" style={{ color: faction.ink }}>
                {faction.label}
              </p>
              <h3 className={`${compact ? "text-base" : "text-lg sm:text-xl"} mt-1 leading-tight font-black text-white`}>{card.name}</h3>
            </div>
          ...
          <div className="mt-2 flex flex-1 flex-col rounded-lg border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(0,0,0,0.3))] p-3">
            <p className={`${compact ? "min-h-12 text-xs" : "min-h-14 text-xs sm:min-h-16 sm:text-sm"} font-bold leading-5 text-white/82`}>{card.effectText}</p>
